...
# The main NBM evaluation is in evaluate_scarce_NBM.py, refer there for further documentation.
###############################


def get_reconstr_errors(model, dl, device, norm_change_f = None):
    '''
    ...
    Args:
        ...
        norm_change_f (torch-adapted function): 
            Takes a batch of samples, unnorms them into raw values, and normalizes them with different statistics.
            Used only in a domain mapping context, where WT data may be normalized with different statistics than used for pretraining the NBM. 
    '''
    ...
    # ii) Calculate the mae between the original and reconstructions
    # iii) Store all reconstruction errors in a dictionary 
    with torch.no_grad():
        for x, _ in iter(dl): # NOTE: the label (wt identified) is irrelevant for NBMs
            x = x.to(device, non_blocking=True) 
            
            if norm_change_f is not None:
                # if set, apply a different normalization scheme to match the NBM
                x = norm_change_f(x)


            reconstr = model(x) 

            mae_err = np.array(torch.mean(torch.abs(x - reconstr), dim=[1,2]).cpu())
            error_dict["mae"].extend(list(mae_err))

    return error_dict


def get_result_statistics(model, dl, device, norm_change_f = None):
    ''' 
    ...
    '''
    error_dict = get_reconstr_errors(model, dl, device, norm_change_f)

    mae_errors = np.asarray(error_dict["mae"])

    mae_mean = np.mean(mae_errors)
    mae_std = np.std(mae_errors)
    mae_q3 = np.quantile(mae_errors, 0.75)
    mae_q1 = np.quantile(mae_errors, 0.25)
...
def binarize_anomaly_scores(scores, threshold):
    '''
    Given continuous anomaly scores, returns the binarized form on whether the score exceeds the threshold or not.
    '''
    return np.int8(scores >= threshold)
