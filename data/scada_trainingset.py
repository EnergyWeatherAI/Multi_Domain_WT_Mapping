...
###
# An instance of a SCADA_Trainingset: Takes a configuration defining the processing 
# of SCADA data from *one WT*. 
# Given a csv file, it is extracted into mainly a *cleaned, filtered* training and validation set of SCADA samples,
# defined to be of certain shape (included channels and length). The data is processed from a csv
# into pandas dataframes, numpy arrays, and torch datasets and dataloaders prepared for training.
#
# NOTE: No test set data is extracted from this class, use SCADA_Testset to extract unfiltered, potentially anomalous samples. 
# The trainingsets are by default set to filter the data, i.e., to clean it into normal samples only. 
###

class SCADA_Trainingset:
    '''
    ...
    def __init__(self, config, wt_label, scada_csv_path, meta_csv_path):
        '''
        ...
        Args:
            config (dict): 
                    Expected config keys and values:
                        ...
                        val_size (float): The validation size, as relative size of the training set (e.g., 0.30 for 30% *of the cleaned training data*)
                        test_size (float): Set the size of the test set w.r.t the full dataset size; this data which will not be considered for extraction here. NOTE: this value must match the one specified to the corresponding SCADA_Testset
                        ...
                        overwrite_stats (dict or None): 
                            By default, the data will be normalized according to the WT's training data statistics. 
                            In our domain mapping work, we will provide stats from one fixed source WT to apply the same normalization across all WTs in the same setup, for which we then provide a dictionary of statistics to use *instead*. 
            
                        bs (int): batch size for the torch *training* dataloaders.
                        bs_val (int): batch size for the torch *validation* dataloaders. Only used in evaluation and by default 128.
        ...
        See the training scripts (train_mapping.py) for usage examples.

        '''

        self.config = config 
        self.wt_label = wt_label # torch one hot encoded
        self.apply_filters = True # NOTE: expectations to filter for training data
    ...
    def get_data(self):
        '''
        Returns a dictionary containing processed SCADA data according to the config.
        '''
        
        # load the needed rated wind speed and rated power for filtering
        self.get_meta_data() 
        
        # load dataframe, split into tr/val, extract sequences (e.g., 12h sequences) (no test set data in this class)
        tr_df, val_df, tr_data, val_data = self.extract_sequences() 

        # obtain a normalization and unnormalization function to be able to (de-)normalize data at any time
        ...
        if self.config.get("overwrite_stats") is None: 
            ...
            self.norm_stats = tr_data["stats"]
        # otherwise, calculate the normalization functions using the explicitly provided statistics 
        else:
            norm_fX, unnorm_fX = self.get_normalization(self.config["overwrite_stats"])
            self.norm_stats = self.config["overwrite_stats"]


        # convert into torch datasets and dataloaders, supply the (not yet applied) normalization function to torch
        tr_ds, val_ds = self.get_torch_datasets(tr_data, val_data, self.wt_label, norm_fX)
        tr_dl, val_dl = self.get_torch_dataloaders(tr_ds, val_ds)

        # for the rated power loss in domain mapping, the rated power is needed as a normalized value as the model only sees normalized samples
        # The selected channel - the average power (ch. 1) - is extracted and converted with to a global normalized value with normfX
        ...
        scada_sample_dataset =  {
                "dfs": {"tr": tr_df, "val": val_df},
                "data": {"tr": tr_data, "val": val_data},
                "torch_datasets": {"tr": tr_ds, "val": val_ds},
                "torch_dataloaders": {"tr": tr_dl, "val": val_dl},
                "norm_stats": self.norm_stats, "norm_fX": norm_fX, "unnorm_fX": unnorm_fX,
    ...
    def extract_sequences(self):
        '''Returns loaded and tr/val dataframes, and extracted sequences (e.g., 12h) obtained using a sliding window approach.'''
        
        # convert the csv file into a large dataframe
        wt_df = self.load_df()
        # split into training and validation sets according to the configuration (sizes and scarcitiy degree)
        tr_df, val_df = self.split_df(wt_df)
        ...
        return tr_df, val_df, tr_data, val_data
    ...
    def split_into_sequences_extra(self, df, convert_to_ch_first = True):
        ...
        for row in range(len(df_dict["Timestamp"])-(self.config["seq_len"]+1)):
            ...
            if np.isnan(x).sum() == 0:
                ...
                # an incident is anything with an incident value above 0
                incident_flags.append(np.array(df_dict["incident"][row:row+self.config["seq_len"]]).max())
                

        
        if len(x_seq) == 0: return None, None, None 
    ...
    def load_df(self, keep_incident_flag=True):
        ...
        return load_csv_as_df(self.scada_csv_path, self.config["x_features"], self.rated_ws, self.rated_pwr, self.apply_filters, keep_incident_flag) 
    ...
    def adjust_statistics(self, stats):
        ...
        '''
        # hard-sets minimum value of 0 for all power, windspeed, and rotorspeed values!
    ...
    def split_df(self, wt_df, verbose=False):
        '''
        Splits a WT SCADA dataframe into (possibly scarce) training and validation dataframes (according to configuration) and returns them.
        '''


        # test size is INDEPENDENT of training (subset-) size. Always the last test_size*100% of data.
        # test set data will be disregarded in this class
        n_test_set = int(self.config["test_size"] *len(wt_df)) 


        # if NO SCARCITY is set, split the entire non-test remainder into a training and validation set
        if self.config["limit_tr_to"] is None:
            ...
            # SCARCITY is selcted by limiting the training data size (includes validation set) to config["limit_tr_to"] sequence samples
            # To have a continuous training and test transition, 
            # only the last -limit_tr_to sequences samples prior to the test set are retained (e.g., the last 2 weeks just BEFORE the test set)

            # find_idx_limit finds the appropriate cutoff to get exactly limit_tr_to samples (non-nan)
        ...
        if verbose: print(f"Original DF lens: {len(tr_df)}, {len(val_df)}") # print out dataframe sizes for checks
        return tr_df, val_df
    ...
    def get_torch_datasets(self, tr_data, val_data, wt_label, transform):
        '''
        ...
        Args:
            ...
            wt_label (one-hot encoded): Required in a StarGAN to specify origin and destination, will be added as a label for each sample
        ...
        '''
        tr_ds, val_ds = torch_data.get_torch_datasets([tr_data["sequences"], val_data["sequences"]], wt_label = wt_label, transform = transform)
        return tr_ds, val_ds


    def get_torch_dataloaders(self, tr_ds, val_ds, drop_last = True):
        '''
        Converts the torch datasets into dataloaders with specific batch sizes and shuffle properties.
        Args:
            tr_ds | val_ds  (torch dataset): A torch dataset object , see get_torch_datasets and torch_data.py
            drop_last (bool): If set to true, the last batch is discarded. Needed for consistent and matching training batch sizes in the mapping.
        '''
        tr_dl, val_dl = torch_data.get_dataloaders([tr_ds, val_ds], batch_sizes=[self.config["bs"], self.config.get("val_bs", 256)], shuffles=[True, False], drop_last = [drop_last, False]) 
        return tr_dl, val_dl
