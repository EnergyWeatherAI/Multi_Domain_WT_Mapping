...
###
# Functions within this file are used to convert SCADA csv files containing measurements and incident flags (see dummy dataset)
...
# Some domain-specific manual cleaning is included; might require manual adaptation to specific user cases.  
###
...
def load_csv_as_df(path, x_features, rated_ws, rated_pwr, filter_incidents = True, keep_incident_flag = False):
    ...
    Args:
        ...
        rated_ws (float): rated wind speed of the respective WT in m/s, required for filtering
        rated_pwr (float): rated power of the respective WT in KW (equal to SCADA-system), required for filtering
        filter_incidents (bool): if True, incidents (based on column operation) and outliers (based on filtering operation) will be excluded. Set to False to get test/evaluation data.
        keep_incident_flag (bool): if True, dataframe will retain the incident column additionally to the x_features columns
    '''
